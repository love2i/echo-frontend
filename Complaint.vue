...
<!--                       :filter-method="filterHandler"-->
   ...
   <!--   新增的查看回复     -->
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button type="primary" @click="inquireReply(scope.row)">查看回复</el-button>
        </template>
      </el-table-column>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      inquireReply() {
      //新增的查看回复
        this.$axios.get('https:///api/web/complaint/getAllReplyById '+this.page.pageNum+'&pageSize='+this.page.pageSize)
            .then(res => {
              console.log(res)
              if (res.success) {
                this.time = res.reply_time
                this.style=res.reply_style
